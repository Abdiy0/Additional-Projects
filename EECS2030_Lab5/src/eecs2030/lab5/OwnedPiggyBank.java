...
public class OwnedPiggyBank {
	...
	public OwnedPiggyBank(OwnedPiggyBank other) {
		...
		this.coin = new ArrayList<Coin>(other.coin);
	...
	public boolean contains(Coin coin) {
		...
		return this.coin.contains(coin);
	...
	public List<Coin> removeCoins(Owner user, int value) {
		...
		ArrayList<Coin> rem = new ArrayList<Coin>();
		int t = value;
		Collections.sort(this.coin);

		Collections.reverse(this.coin);

		if (!(this.owner.equals(user))) {
			return this.coin = new ArrayList<Coin>();
		}

		for (int i = 0; i < this.coin.size(); i++) {

			if (this.coin.get(i).getValue() <= t) {

				rem.add(new Coin(this.coin.get(i)));
				t -= this.coin.get(i).getValue();
				this.coin.remove(i);
				i--;
			}
		}
	...
	public List<Coin> deepCopy() {
		...
		ArrayList<Coin> deepcoin = new ArrayList<Coin>();

		for (Coin c : this.coin) {
			deepcoin.add(new Coin(c));
		}

		Collections.sort(deepcoin);
